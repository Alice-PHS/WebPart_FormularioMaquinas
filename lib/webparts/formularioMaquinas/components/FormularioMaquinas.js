...
export default function FormularioMaquinas(props) {
    var _this = this;
    var _a = useState(null), mode = _a[0], setMode = _a[1];
    var _b = useState(''), chamado = _b[0], setChamado = _b[1];
    var _c = useState(''), nomeEmpresa = _c[0], setNomeEmpresa = _c[1];
    var _d = useState(''), solicitanteEmail = _d[0], setSolicitanteEmail = _d[1];
    var _e = useState(''), tag = _e[0], setTag = _e[1];
    var _f = useState(''), hostname = _f[0], setHostname = _f[1];
    var _g = useState(''), equCodigo = _g[0], setEquCodigo = _g[1];
    var _h = useState(''), departamento = _h[0], setDepartamento = _h[1];
    var _j = useState([]), maquinas = _j[0], setMaquinas = _j[1];
    // NOVO ESTADO: Controle de acesso
    var _k = useState(true), isAuthorized = _k[0], setIsAuthorized = _k[1];
    var buscarEmpresaPorDominio = function (dominio) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    return [4 /*yield*/, props.context.spHttpClient.get(query, SPHttpClient.configurations.v1)];
                case 1:
                    response = _b.sent();
    ...
    var renderContent = function () {
        ...
        switch (mode) {
            case 'inclusao': return React.createElement(FormInclusao, { numeroChamado: chamado, nomeEmpresa: nomeEmpresa, solicitanteEmail: solicitanteEmail });
            case 'exclusao': return React.createElement(FormExclusao, { numeroChamado: chamado, nomeEmpresa: nomeEmpresa, solicitanteEmail: solicitanteEmail, tag: tag, hostname: hostname, equCodigo: equCodigo, departamento: departamento, maquinas: maquinas });
            case 'substituicao': return React.createElement(FormSubstituicao, { numeroChamado: chamado, nomeEmpresa: nomeEmpresa, solicitanteEmail: solicitanteEmail, tag: tag, hostname: hostname, equCodigo: equCodigo, departamento: departamento, maquinas: maquinas });
            case 'novoUsuario': return React.createElement(FormNovoUsuario, { numeroChamado: chamado, nomeEmpresa: nomeEmpresa, solicitanteEmail: solicitanteEmail });
            default:
                return (React.createElement("div", { style: { minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Segoe UI', system-ui, sans-serif" } },
